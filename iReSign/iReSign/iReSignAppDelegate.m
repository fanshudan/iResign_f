...
- (void)doProvisioning {
    ...
    targetPaths = [NSMutableArray array];
    for (NSString *file in dirContents) {
        if ([[[file pathExtension] lowercaseString] isEqualToString:@"app"]) {
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:[appPath stringByAppendingPathComponent:@"embedded.mobileprovision"]]) {
                ...
                [targetPaths addObject:[appPath stringByAppendingPathComponent:@"embedded.mobileprovision"]];
                break;
            }
        }
    }
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:plugInsDirPath]) {
                ...
                for (NSString *plugInFile in plugInsContents) {
                    ...
                    if ([extension isEqualTo:@"appex"] ) {
                        ...
                        if ([[NSFileManager defaultManager] fileExistsAtPath:[plugInPath stringByAppendingPathComponent:@"embedded.mobileprovision"]]) {
                            NSLog(@"Found embedded.mobileprovision, deleting.");
                            [[NSFileManager defaultManager] removeItemAtPath:[plugInPath stringByAppendingPathComponent:@"embedded.mobileprovision"] error:nil];
                            [targetPaths addObject:[plugInPath stringByAppendingPathComponent:@"embedded.mobileprovision"]];
                        }
                    }
                }
            }
    ...
    if ([provisioningTask isRunning] == 0) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:plugInsDirPath]) {
                    ...
                    for (NSString *plugInFile in plugInsContents) {
                        ...
                        if ([extension isEqualTo:@"appex"] ) {
                            ...
                            NSLog(@"Found plugInPath :%@",plugInPath);
                            if ([[NSFileManager defaultManager] fileExistsAtPath:[plugInPath stringByAppendingPathComponent:@"embedded.mobileprovision"]]) {
                                if ([targetPaths containsObject:[plugInPath stringByAppendingPathComponent:@"embedded.mobileprovision"]]) {
                                   [targetPaths removeObject:[plugInPath stringByAppendingPathComponent:@"embedded.mobileprovision"]];
                                }
                            }else{
        ...
        for (NSString *file in dirContents) {
            if ([[[file pathExtension] lowercaseString] isEqualToString:@"app"]) {
                appPath = [[workingPath stringByAppendingPathComponent:kPayloadDirName] stringByAppendingPathComponent:file];
                if ([[NSFileManager defaultManager] fileExistsAtPath:[appPath stringByAppendingPathComponent:@"embedded.mobileprovision"]]) {
                    if ([targetPaths containsObject:[appPath stringByAppendingPathComponent:@"embedded.mobileprovision"]]) {
                       [targetPaths removeObject:[appPath stringByAppendingPathComponent:@"embedded.mobileprovision"]];
                    }
                    if (targetPaths.count == 0) {
                       [self checkMobileprovisionHelper:appPath];
                    }
                } else {
                    [self showAlertOfKind:NSCriticalAlertStyle WithTitle:@"Error" AndMessage:@"Provisioning failed"];
                    [self enableControls];
...
- (void)checkMobileprovisionHelper:(NSString *)path{
     BOOL identifierOK = FALSE;
     NSString *identifierInProvisioning = @"";
     
     NSString *embeddedProvisioning = [NSString stringWithContentsOfFile:[path stringByAppendingPathComponent:@"embedded.mobileprovision"] encoding:NSASCIIStringEncoding error:nil];
...
- (void)doCodeSigning {
    ...
    if (appPath) {
        if (hasFrameworks) {
            ...
            [self signFile:[plugIns lastObject]];
            [plugIns removeLastObject];
        } else{
...
- (void)checkCodesigning:(NSTimer *)timer {
    if ([codesignTask isRunning] == 0) {
        ...
        if (frameworks.count > 0) {
            ...
            [self signFile:[plugIns lastObject]];
            [plugIns removeLastObject];
        }else if (hasPlugIns || hasFrameworks) {
            hasPlugIns = NO;
            hasFrameworks = NO;
            [self signFile:appPath];
        }else {
